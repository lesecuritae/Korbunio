...
def test_retailer_markets_resolve_every_requested_catalog_retailer():
    class RetailerHttp(FakeHttp):
        def get_bytes(self, url, headers=None):
            ...
            return json.dumps([{"osm_id": name, "lat": "51.051", "lon": "13.741",
                "display_name": f"{name}, Teststraße 1, 01067 Dresden",
                "address": {"postcode": "01067", "shop": name}, "namedetails": {"name": name},
                "extratags": {"website": "https://example.test/"}}]).encode()

    resolver = AldiRegionResolver(RetailerHttp())
    resolver.http = RetailerHttp()
    markets = resolver.retailer_markets("01067", ("Lidl", "dm"))
    assert {market["retailer"] for market in markets} == {"Lidl", "dm"}
    assert all(market["postal_code"] == "01067" for market in markets)
...
def test_retailer_markets_classify_plain_aldi_name_from_provider_metadata():
    class AldiHttp(FakeHttp):
        def get_bytes(self, url, headers=None):
            ...
            if "postalcode=" in url:
                return json.dumps([{"lat": "51.05", "lon": "13.74"}]).encode()
            return json.dumps([{"osm_id": "test-aldi", "lat": "51.051", "lon": "13.741",
                "display_name": "Aldi, Teststraße 1, 01067 Dresden",
                "address": {"postcode": "01067", "shop": "Aldi"}, "namedetails": {"name": "Aldi"},
                "extratags": {"website": "https://www.aldi-nord.de/filiale/test"}}]).encode()

    resolver = AldiRegionResolver(AldiHttp())
    resolver.http = AldiHttp()
    markets = resolver.retailer_markets("01067", ("ALDI Nord", "ALDI Süd"))
    assert [(market["market_id"], market["retailer"]) for market in markets] == [("test-aldi", "ALDI Nord")]
